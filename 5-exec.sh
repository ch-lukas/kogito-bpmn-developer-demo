...
#
# Usage:
#   ./5-exec.sh url                          # print base URL + Swagger
...
#   ./5-exec.sh tasks  [process-id] <iid>    # list tasks waiting on humans
#   ./5-exec.sh complete [process-id] <iid> <task-name> <tid> [json]
...
Run './5-exec.sh ls' to list deployments, or check 'Dev Deployments ▾' in the editor."
  fi
  printf '%s' "$prefix"
}
